...
export const useGenerationValidation = ({ modelSettings, stationPrompts }: GenerationValidationProps) => {
  ...
  const getGenerationDisabledState = useMemo(() => {
    return (stationType: 'adCopy' | 'landingPage' | 'customRequest' | 'email' | 'sms' | 'staticAd') => {
      ...
      switch (stationType) {
        ...
        case 'email':
          ...
          stationPromptReason = 'Email Retention system prompt is not configured';
          break;
        case 'sms':
          hasStationPrompt = stationPrompts?.sms?.systemPrompt && stationPrompts.sms.systemPrompt.trim() !== '';
          stationPromptReason = 'SMS Retention system prompt is not configured';
          break;
        case 'staticAd':
