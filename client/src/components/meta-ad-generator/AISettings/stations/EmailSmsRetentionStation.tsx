...
export const EmailSmsRetentionStation: React.FC<EmailSmsRetentionStationProps> = ({ editingConfig, setEditingConfig, effectiveUser, expandedStations, setExpandedStations, copyToClipboard }) => {
  ...
  return (
    <div className="border border-gray-200 rounded-lg">
      <StationToggleButton isOpen={expandedStations.has('emailSmsRetention')} onClick={() => toggleStation('emailSmsRetention')} title="Email & SMS Retention Station" icon={<Mail className="w-5 h-5" />} iconColor="text-orange-500" description="Customer retention and engagement specialist" />
      {expandedStations.has('emailSmsRetention') && (
        <div className="p-6 pt-4 border-t border-gray-100 space-y-6">
          <ProtectedPromptEditor label="System Prompt" value={editingConfig?.stationPrompts?.emailSmsRetention?.systemPrompt || ''} onChange={(value: string) => setEditingConfig({...editingConfig, stationPrompts: {...editingConfig?.stationPrompts, emailSmsRetention: {...editingConfig?.stationPrompts?.emailSmsRetention, systemPrompt: value}}})} placeholder="You are an email and SMS marketing specialist focused on customer retention and engagement..." rows={6} disabled={effectiveUser?.role !== 'admin'} copyToClipboard={copyToClipboard} />
          <EnhancedTextField label="User Prompt Template" value={editingConfig?.stationPrompts?.emailSmsRetention?.userPromptTemplate || ''} onChange={(value: string) => effectiveUser?.role === 'admin' && setEditingConfig({...editingConfig, stationPrompts: {...editingConfig?.stationPrompts, emailSmsRetention: {...editingConfig?.stationPrompts?.emailSmsRetention, userPromptTemplate: value}}})} placeholder="Create [EMAIL/SMS] retention copy for [CAMPAIGN_TYPE] targeting [AUDIENCE_SEGMENT]..." rows={4} disabled={effectiveUser?.role !== 'admin'} bgColor="bg-green-50" borderColor="border-green-200" copyToClipboard={copyToClipboard} />
          ...
          <EnhancedArrayField label="Retention Best Practices" value={editingConfig?.stationPrompts?.emailSmsRetention?.retentionBestPractices || []} onChange={(value: string[]) => effectiveUser?.role === 'admin' && setEditingConfig({...editingConfig, stationPrompts: {...editingConfig?.stationPrompts, emailSmsRetention: {...editingConfig?.stationPrompts?.emailSmsRetention, retentionBestPractices: value}}})} placeholder="Send times: Email 10-11am EST, SMS 2-4pm EST&#10;Frequency: Email 2-3x/week max, SMS 1-2x/week max&#10;Personalization: Use first name and purchase history" rows={5} disabled={effectiveUser?.role !== 'admin'} bgColor="bg-amber-50" borderColor="border-amber-200" copyToClipboard={copyToClipboard} />
        </div>
      )}
    </div>
  );
