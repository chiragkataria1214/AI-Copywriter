...
import { SubpersonaManager } from '@/components/main/ai-settings/SubpersonaManager';
...
interface PersonasTabProps {
  ...
  onSavePersonaPillars?: (pp: TrainingConfig['personaPillars']) => void;
  onSaveSinglePersona?: (args: { personaName: string; personaData: any }) => void;
}
      ...
      {effectiveUser?.role === 'admin' && (
        <div className="border border-blue-200 rounded-lg p-3 bg-blue-50">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h4 className="text-sm font-semibold text-blue-900">Add New Persona</h4>
              <p className="text-xs text-blue-700">Create a target persona with custom pillars</p>
            </div>
          </div>
          ...
          {newPersonaName.trim() && (
            <div className="mt-2 p-2 bg-white/70 rounded-md border border-blue-200">
                {editingConfig?.personaPillars?.[newPersonaName.trim()] ? (
                  ...
                  <p className="text-xs text-blue-700">
                    <span className="font-medium">Preview:</span> Will create persona "{newPersonaName}" with empty pillars you can customize.
                  </p>
                )}
            </div>
          )}
                ...
                <div className="p-4 pt-3 border-t border-gray-100">
                  <div className="space-y-3">
                    {!personaData.description && effectiveUser?.role === 'admin' && (
                      <div className="mb-4">
                        <Label className="text-xs font-medium text-gray-900 mb-1.5 block">
                          Persona Description
                        </Label>
                        <Textarea
                          value=""
                          onChange={(e) => {
                            ...
                            setEditingConfig({
                              ...
                              personaPillars: {
                                ...editingConfig.personaPillars,
                                [personaName]: updatedPersona
                              }
                            });
                            onSaveSinglePersona?.({ personaName, personaData: updatedPersona });
                          }}
                          className="text-gray-900 border-blue-200 focus:border-blue-400 text-sm"
                          rows={2}
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gray-600 mb-2">
                        Define what matters most to this persona. Used to guide copy.
                      </p>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
                        {(personaData.pillars || ['']).map((pillar: string, index: number) => (
                          <div key={index} className={`rounded-md border ${personaData.enabledPillars?.[index] === false ? 'opacity-60' : ''} border-gray-200 bg-white/60 p-2`}>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              />
                              <span className="text-[11px] text-gray-600">Pillar {index + 1}</span>
                              {effectiveUser?.role === 'admin' && (personaData.pillars?.length > 1) && (
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  className="text-red-500 hover:text-red-700 ml-auto h-6 w-6"
                                  onClick={() => {
                            ...
                            <Input
                              ...
                              className={`w-full ml-0 text-gray-900 font-medium border-blue-200 focus:border-blue-400 text-sm`}
                              placeholder={`Enter pillar ${index + 1} (e.g., lack of time, versatility, clean ingredients)`}
                        ...
                        {effectiveUser?.role === 'admin' && (
                          <div className="md:col-span-2">
                            <Button
                              ...
                              className="w-full border-dashed border-blue-300 text-blue-600 hover:bg-blue-50 mt-2 py-1.5 text-xs"
                            >
                    ...
                    {/* Subpersonas Section */}
                    {personas && personas[personaName] && personas[personaName].id && (
                      <SubpersonaManager
                        personaId={personas[personaName].id}
                        personaName={personaName}
