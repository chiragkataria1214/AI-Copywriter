...
import { Upload, Copy, Check, Target, Sparkles, Camera, FileText, AlertCircle, Settings, Heart, MessageCircle, Send, Bookmark, MoreHorizontal, Globe, ThumbsUp, MessageSquare, Share2, Music, Volume2, Play, User as UserIcon, ChevronLeft, ChevronRight, TrendingUp, Users, Zap, Calendar, BarChart3, Lightbulb, ChevronDown, ChevronUp, Eye } from 'lucide-react';
import {
    Accordion,
    AccordionContent,
    AccordionItem,
...
export function OrganicSocialTab({
  ...
  const handlePrevCaption = () => {
    setActiveCaptionIndex(prev => Math.max(0, prev - 1));
  };

  const handleNextCaption = () => {
    if (generatedCaptions) {
      setActiveCaptionIndex(prev => Math.min(generatedCaptions.length - 1, prev + 1));
    }
  };
  ...
  const insightIcons: { [key: string]: React.ElementType } = {
    hashtagCompetitionAnalysis: TrendingUp,
    engagementOptimizationStrategy: Heart,
    viralPotentialAssessment: Zap,
    audienceGrowthStrategy: Users,
    contentSyndicationPlan: Share2,
    seasonalPerformanceInsights: Calendar,
    communityBuildingStrategy: MessageCircle,
    contentSeriesOpportunities: FileText,
    influencerActivationGuidance: UserIcon,
    algorithmOptimizationTactics: BarChart3,
    trendJackingOpportunities: TrendingUp,
    crisisCommunicationReadiness: AlertCircle,
    performanceTrackingFramework: BarChart3,
    contentRefreshSchedule: Lightbulb,
    default: Lightbulb,
  };
  ...
  // Helper function to render strategic insights
  const renderStrategicInsights = (insights: Record<string, any>) => {
    return (
        ...
        <div className="border border-gray-200 rounded-lg bg-white">
          <div className="px-4 pt-3 pb-2 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div className="text-sm font-semibold text-gray-900">jonesroadbeauty</div>
            </div>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    // Store strategic insights if available
                    console.log('🔍 API Response data:', data);
                    console.log('📊 Strategic Insights in response:', data.strategicInsights);
                    
                    if (data.strategicInsights) {
                      console.log('✅ Setting strategic insights:', data.strategicInsights);
                      setStrategicInsights(data.strategicInsights);
                    } else {
                      console.log('❌ No strategic insights found in response');
                      setStrategicInsights(null);
                    }
                  } catch (error) {
                ...
                <CardContent className="p-6">
                  {isGeneratingCaptions ? (
                    ...
                    <div>
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Button
                                  variant="outline"
                                  onClick={() => {
                                    handleCopyToClipboard(generatedCaptions[activeCaptionIndex], `variation-${activeCaptionIndex}`);
                                  }}
                                  className="flex items-center space-x-2"
                                  size="icon"
                                  disabled={copiedStates[`variation-${activeCaptionIndex}`]}
                                >
                                  {copiedStates[`variation-${activeCaptionIndex}`] ? <Check size={16} /> : <Copy size={16} />}
                                </Button>
                              </TooltipTrigger>
                              <TooltipContent>
                                {copiedStates[`variation-${activeCaptionIndex}`] ? 'Copied!' : 'Copy'}
                              </TooltipContent>
                            </Tooltip>
                          </TooltipProvider>
                          <Button
                        ...
                        <div className="border rounded-lg p-4 bg-gray-50/70">
                          <div className="flex justify-between items-center mb-3">
                            <h4 className="font-bold text-lg text-gray-800">
                              Variation {activeCaptionIndex + 1}
                            </h4>
                            <div className="flex items-center space-x-2">
                              <Button variant="ghost" size="icon" onClick={handlePrevCaption} disabled={activeCaptionIndex === 0} className="h-8 w-8">
                              ...
                              <div className="flex items-center space-x-2">
                                {generatedCaptions.map((_, index) => (
                                  <button
                                    ...
                                    className={`w-2 h-2 rounded-full transition-colors ${activeCaptionIndex === index ? 'bg-jones-primary' : 'bg-gray-300 hover:bg-gray-400'}`}
                                  />
                                ))}
                              </div>
                              <Button variant="ghost" size="icon" onClick={handleNextCaption} disabled={activeCaptionIndex === generatedCaptions.length - 1} className="h-8 w-8">
                          ...
                          <div className="border-t pt-4 space-y-4">
                            {(() => {
                                const caption = generatedCaptions[activeCaptionIndex];
                                ...
                                if (!isMulti) {
                                  ...
                                  return (
                                    <div className="relative">
                                      <button
                                        onClick={() => handleCopyToClipboard(caption, `platform-post-${selectedPlatform}-${activeCaptionIndex}`)}
                                        className="absolute right-3 top-3 z-10 text-jones-primary hover:text-jones-secondary"
                                        aria-label="Copy caption"
                                        disabled={copiedStates[`platform-post-${selectedPlatform}-${activeCaptionIndex}`]}
                                      >
                                        {copiedStates[`platform-post-${selectedPlatform}-${activeCaptionIndex}`] ? <Check size={16} /> : <Copy size={16} />}
                                      </button>
                                      {renderPlatformPost({ platform: selectedPlatform, caption, imageSrc, captionIndex: activeCaptionIndex })}
                                    </div>
                                  );
                                }
                                ...
                                return (
                                  <div className="space-y-4">
                                    <div className="text-sm font-medium text-gray-700">Caption {activeCaptionIndex + 1} (Multi-Platform)</div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                      {platforms.map((p) => (
                                        <div key={p} className="relative">
                                          <button
                                            onClick={() => handleCopyToClipboard(caption, `platform-post-${p}-${activeCaptionIndex}`)}
                                            className="absolute right-3 top-3 z-10 text-jones-primary hover:text-jones-secondary"
                                            aria-label={`Copy ${p} caption`}
                                            disabled={copiedStates[`platform-post-${p}-${activeCaptionIndex}`]}
                                          >
                                            {copiedStates[`platform-post-${p}-${activeCaptionIndex}`] ? <Check size={16} /> : <Copy size={16} />}
                                          </button>
                                          {renderPlatformPost({ platform: p, caption, imageSrc, captionIndex: activeCaptionIndex })}
                                        </div>
                                      ))}
                                    </div>
                                  </div>
                      ...
                      {strategicInsights && renderStrategicInsights(strategicInsights)}
                    </div>
                  ) : (
                    <div className="text-center py-16">
                          ...
                          <div className="relative bg-black border-[10px] border-gray-800 rounded-[40px] shadow-2xl overflow-hidden h-[580px]">
                            <div className="absolute top-0 left-0 right-0 z-20 px-3 pt-3">
                              ...
                              <div className="flex items-center space-x-2 mt-2">
                                ...
                                <div className="text-white text-sm font-semibold">jonesroadbeauty</div>
                                  ...
                                  <TooltipTrigger asChild>
                                    <button
                                      onClick={() => {
                                        ...
                                        handleCopyToClipboard(slideContent, `slide-${currentStorySlide}`);
                                      }}
                                      className="text-jones-primary hover:bg-jones-light p-1 rounded-md flex items-center space-x-1.5 text-xs font-medium"
                                      disabled={copiedStates[`slide-${currentStorySlide}`]}
                                    >
                                      {copiedStates[`slide-${currentStorySlide}`] ? <Check size={14} /> : <Copy size={14} />}
                                    </button>
                                  </TooltipTrigger>
                                  <TooltipContent>
                                    {copiedStates[`slide-${currentStorySlide}`] ? 'Copied' : 'Copy Slide'}
                                  </TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                            </div>
