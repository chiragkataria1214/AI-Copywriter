...
**Latest Update - Station Prompts Optimization & Product Claims Fix (July 30, 2025)**
- **✅ HEADLINE FRAMEWORK DROPDOWN**: Station prompts now use dropdown selection from Copy Frameworks->Headline Frameworks instead of manual input
- **✅ REMOVED DUPLICATE GUIDELINES**: Eliminated Brand-First and Direct Response Guidelines from station prompts - now uses Copy Frameworks configuration exclusively
- **✅ STREAMLINED STATION CONFIG**: Station prompts reference Copy Frameworks tab for brand/DR guidelines with helpful user guidance
- **✅ PRODUCT DELETION FIXED**: Resolved bug where delete product functionality only worked for new products, not existing ones
...
- **✅ DELETE PRODUCTS**: Admin can remove products from claims management with single click (now working for all products)
