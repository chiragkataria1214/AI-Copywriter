...
import { z } from 'zod';
import { importFromJunip, importFromText, type ImportResult } from '../services/review-importer';
import { createJunipClient } from '../services/junip-api';
import { importJunipReviewsFromPage } from '../services/junip-web-scraper';
import { db } from '../db';
import { reviews, reviewAnalysis, trainingInsights } from '@shared/review-schema';
import { sql, desc, eq, count } from 'drizzle-orm';

// Generate training insights from imported reviews
...
export function registerJunipRoutes(app: Express) {
  ...
  app.post('/api/junip/import-page', async (req, res) => {
    try {
      ...
      const { importJunipReviewsFromPage } = await import('../services/comprehensive-junip-scraper');
