  ...
  app.post('/api/fetch-drive-brief', requireAuth, async (req, res) => {
    try {
      ...
      const exportUrls = [
        ...
        `https://docs.google.com/document/d/${docId}/export?format=txt&gid=0`,
      ...
      if (!content || content.trim().length === 0) {
        return res.status(400).json({ 
          message: `Could not access document (${lastError}). Please ensure:\n1. Document is shared with "Anyone with the link can view"\n2. Document is not empty\n3. You're using a Google Docs URL (not Sheets or Slides)`
        });
      }
